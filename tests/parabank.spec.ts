...
test('Register new user', async ({ page }) => {
  ...
  const edtFirstName = page.locator('[id="customer.firstName"]');
  await expect(edtFirstName).toBeEditable();
  if (await edtFirstName.isVisible()) {
    await page.locator('[id="customer.firstName"]').fill('Test_FName_5');  
    await page.locator('[id="customer.lastName"]').fill('Test_LName_5');
    ...
    await page.locator('[id="customer.username"]').fill('TestUser_005');
    await page.locator('[id="customer.password"]').fill('TempPwd');
    await page.locator('#repeatedPassword').fill('TempPwd');
    await page.getByRole('button', { name: 'Register' }).click();
    await expect(page.getByRole('heading', { name: 'Welcome TestUser_005' })).toBeVisible();  
  };
});
test('Login with existing user', async ({ page }) => {
  await page.goto('https://parabank.parasoft.com/parabank/index.htm');
  await page.locator('input[name="username"]').fill('TestUser_005');
